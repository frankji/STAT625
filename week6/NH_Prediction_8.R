...
## PRELIMINARY WORK:
hist(log(x$totval)) ### dj333 ### Considering log value?
...
### dj333 ### It might be better if the object is not named with an existed function name.
...
for (pid in x$pid) { 
  ans <- IsItFair(x, pid)
  if (!is.na(ans$estval)) {
    zzz$estval[pid] <- ans$estval ### dj333 ### Repeated assignments, try lapply and sapply.
...
write.csv(zzz, "NH_Prediction_8.csv", row.names=FALSE) ### dj333 ### spacing "row.names = FALSE".
...
zzz$estval <- as.numeric(predict(lm, newdata = test))
...
write.table(zzz, file = 'NH_Predtest_8.csv', sep = ',', row.names = FALSE)