...
info[, 'finalbaths'] <- apply(info[, c('bathrooms', 'halfbaths')], 1, function(x) sum(c(x[1], 0.5*x[2]), na.rm = TRUE))
info[is.na(info[, "bathrooms"]) & is.na(info[, "halfbaths"]), "finalbaths"] <- NA
