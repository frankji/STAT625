...
Events <- c('M3mSB', 'W3mSB', 'M10mPF', 'W10mPF')
events <- c("Men's 3m Springboard", "Women's 3m Springboard", "Men's 10m Platform", "Women's 10m Platform")
getfirst <- function(txt, label, t) {
  first <- grep(label, txt, perl = TRUE)
  min(first[first - t > 0])
}
dive <- c(6, 5, 6, 5)
j.panels <- matrix(NA, nrow = 4, ncol = 3)
j.panels[1, ] <- c(1, 2, 5)
j.panels[c(2,3,4), ] <- matrix(rep(c(1, 2, 3), each = 3), nrow = 3)
cap <- function(x) gsub("([A-Z])([A-Za-z]+)", "\\1\\L\\2", x, perl=TRUE)
for (m in 1:length(Events)){
  m3m <- grep(events[m], myfile, perl = TRUE)
  Event <- Events[m]
  ...
  sink(paste(Event, '_judges.txt', sep = ''))
  ...
  judge.info <- scan(file = paste(Event, '_judges.txt', sep = ''), what = character(), sep = '\n')
  ...
  write.table(judges, file = paste(Event, '_judges.csv', sep = ''), sep = ',', row.names = FALSE)
  ###########################################################################
  
  Judges <- read.csv(paste(Event, '_judges.csv', sep = ''), as.is = TRUE)
  sink(paste(Event, '_divers.txt', sep = ''))
  ...
  m3m.info <- scan(file = paste(Event, '_divers.txt', sep = ''), what = character(), sep = '\n')
  ...
  m3m.info <- scan(file = paste(Event, '.txt', sep = ''), what = character(), sep = '\n')
  ...
  for (i in 1:3) {
    ...
    panel <- ifelse(preornot, temp.table$DiveNo %in% panels, rep(TRUE, nrow(temp.table)))
  ...
  write.table(finaltab, file = paste(Event, '.csv', sep = ''), sep = ',', row.names = FALSE)
}
Diving <- lapply(Events, function(x) read.csv(paste(x, '.csv', sep = ''), as.is = TRUE))
Diving <- do.call(rbind, Diving)
Diving$JCountry[Diving$Judge == 'BRUN Aristid'] <- 'AUS'
Diving$JCountry[Diving$Judge == 'ROCHA Sergio'] <- 'MEX'
Diving$JCountry[Diving$Judge == 'CHONG HON KEO Kelvin'] <- 'MAS'
write.table(Diving, file = 'Diving2010.csv', sep = ',', row.names = FALSE)