...
Judges <- read.csv('w3m_judges.csv', as.is = TRUE)
sink("w3m_divers.txt")
...
m3m.info <- scan(file = 'w3m_divers.txt', what = character(), sep = '\n')
...
m3m.info <- unique(scan(file = 'w3m.txt', what = character(), sep = '\n'))
...
first.pat <- paste(paste(divers.cap, following, sep = ''), collapse = '|')
temp <- grep(first.pat, m3m.info, perl = TRUE)
tables <- list()
for(i in 1:(length(starts)-1)){
  selected <- temp[temp > starts[i] & temp < starts[i+1]]
  if (length(selected) > 0) {
    tables[[i]] <- m3m.info[as.vector(sapply(selected, function(x) seq(x, x+4)))]
  } else tables[[i]] <- NA
}
...
B <- grep('BENEDETTI Michele', m3m.info)
B <- max(B[667 - B > 0])
B.ends <- getfirst(txt = m3m.info, t = B, label = 'Note') - 1
tables[[3]] <- c(tables[[3]], m3m.info[(B + 6):B.ends])
#######################################################
