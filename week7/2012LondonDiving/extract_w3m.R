...
m3m <- grep("Women's 3m Springboard", myfile, perl = TRUE)
Event <- 'W3mSB'
...
judge.info <- scan(file = 'W3mSB_judges.txt', what = character(), sep = '\n')
...
write.table(judges, file = 'W3mSB_judges.csv', sep = ',', row.names = FALSE)
###########################################################################

Judges <- read.csv('W3mSB_judges.csv', as.is = TRUE)
sink("W3mSB_divers.txt")
...
m3m.info <- scan(file = 'W3mSB_divers.txt', what = character(), sep = '\n')
...
for(i in 1:length(pos)){
  m3m.info <- paste(myfile[pos[i]:n.page[i]], collapse = '\n')
...
gettab <- function(x, dives) {
  ...
  if (length(grep('([0-9]+[A-Za-z]+,){2,}', x)) == 1) {
    ...
    tab <- matrix(grep('^[0-9\\.]+$', dat[-c(1, 2, 3, lst + 3)], value = TRUE), nrow = 6)
...
#Something's wrong with the term of 'BENEDETTI Michele'
B <- grep('12,BENEDETTI Michele,[A-Z]{3},[0-9]+[A-Z]', m3m.info)
B.ends <- getfirst(txt = m3m.info, t = B, label = '[^0-9 ,]{2,}') - 1
tables[[3]] <- c(tables[[3]], paste(m3m.info[B:B.ends], collapse = ','))
#######################################################
...
write.table(finaltab, file = 'W3mSB.csv', sep = ',', row.names = FALSE)