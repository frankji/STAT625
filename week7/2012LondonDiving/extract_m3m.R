...
m3m <- grep("Men's 3m Springboard", myfile, perl = TRUE)
Event <- 'M3mSB'
...
judge.info <- scan(file = 'M3mSB_judges.txt', what = character(), sep = '\n')
...
write.table(judges, file = 'M3mSB_judges.csv', sep = ',', row.names = FALSE)
###########################################################################

Judges <- read.csv('M3mSB_judges.csv', as.is = TRUE)
sink("M3mSB_divers.txt")
...
m3m.info <- scan(file = 'M3mSB_divers.txt', what = character(), sep = '\n')
...
m3m.info <- unique(scan(file = 'M3mSB.txt', what = character(), sep = '\n'))
...
write.table(finaltab, file = 'M3mSB.csv', sep = ',', row.names = FALSE)